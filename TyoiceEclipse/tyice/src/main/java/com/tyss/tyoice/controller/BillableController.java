...
import com.tyss.tyoice.dto.Billable;
import com.tyss.tyoice.dto.BillableResponse;
import com.tyss.tyoice.service.BillableService;
...
public class BillableController {
	...
	public BillableResponse addBillableEmp(@RequestBody Billable bill) {
		...
		if (service.insert(bill)) {
			...
			response.setDescription("failed to store data..");
			response.getEmpBill();
		}
		return response;
	}
	...
	public BillableResponse updateBillable(@RequestBody Billable bill) {
		...
		if (service.update(bill)) {
			...
			response.setStatusCode(400);
			response.setMessage("failure");
			response.setDescription("failed to update data..");
		}
		return response;
	}
	...
	public BillableResponse getAllBill() {
		BillableResponse response = new BillableResponse();
		Set<String> list = service.getBillable();
		if (list == null) {
			response.setStatusCode(400);
			response.setMessage("failure");
		} else {
	...
	public BillableResponse getExperience() {
		BillableResponse response = new BillableResponse();
		Set<Integer> list = service.getExperinceList();
		if (list == null) {
			response.setStatusCode(400);
			response.setMessage("failure");
		} else {
	...
	public BillableResponse getByTechnology(@PathVariable String tech) {
		BillableResponse response = new BillableResponse();
		List<Billable> techSet = service.getByTechno(tech);
		if (techSet == null) {
			response.setStatusCode(400);
			response.setMessage("failure");
		} else {
			response.setStatusCode(201);
			response.setBillList(techSet);
		}
		return response;
	}
